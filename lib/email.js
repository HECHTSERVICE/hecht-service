...
/**
 * Генерує PDF-сертифікат гарантії як Buffer.
 * Код ідентичний старому /api/send-email/route.js (свідомо — щоб
 * не змінити верстку PDF під час міграції).
 *
 * @param {object} data - поля сертифікату
...
async function generatePDFBuffer(data) {
  ...
  try {
    ...
    console.warn('[email] Font load failed', e);
  }
...
/**
 * Надсилає 2 листи після реєстрації гарантії:
 *   1) клієнту — сертифікат у PDF-вкладенні + HTML-лист
 *   2) адміну на garantiya@ — нотифікація + PDF
 *
 * Помилки кидає назовні — викликач вирішує що робити
 * (у /api/warranty/register — ловимо і повертаємо emailSent:false,
 * гарантію не видаляємо).
 *
 * @param {object} data
 * @returns {Promise<{ success: boolean }>}
 */
export async function sendWarrantyEmail(data) {
  const { certNumber, firstName, lastName, email, phone, model, serialNumber, purchaseDate } = data;

  if (!certNumber || !email) {
    throw new Error('[email] sendWarrantyEmail: missing certNumber or email');
  }
  ...
  try {
    ...
    console.error('[email] PDF generation error:', pdfErr);
  }
